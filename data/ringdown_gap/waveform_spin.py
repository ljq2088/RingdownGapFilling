...
def hpc(f_,paramater,dw,dtau,Philm = pi/3):
    ...
    #这里改为加上自旋的振幅
    A10 = eta*0.571+0.303#22
    A20 = eta*0.099+0.06#21
    A30 = eta*0.157+0.671#33
    A40 = 0.122 -0.188*eta -0.964*eta**2#44

    kai_p = (mass_ratio*kai1 + kai2)/((mass_ratio+1)*0.9)
    kai_m = (mass_ratio*kai1 - kai2)/((mass_ratio+1)*0.9)
    Adelta = (mass_ratio-1)/(mass_ratio+1)
    A1s = eta*kai_p*(-0.07+0.255/mass_ratio+0.189*mass_ratio-0.013*mass_ratio**2)+0.084*Adelta*kai_m
    A2s = -0.067*kai_m
    A3s = eta*kai_m*(0.163-0.187/mass_ratio+0.021*mass_ratio)+0.073*Adelta*kai_p
    A4s = eta*kai_p*(-0.207/mass_ratio + 0.034*mass_ratio) + Adelta*eta*kai_m*(-0.701+1.387/mass_ratio+0.122*mass_ratio)
    A1 = A10+A1s
    A2 = A20+A2s
    A3 = A30+A3s
    A4 = A40+A4s
...
def sfb(freq,paramater,delta_w,delta_tau,iot = pi/4):
    ...
    w3 = (f1[3] + f2[3]*(1-j)**f3[3])/M *(1+dw3); 
    w4 = (f1[4] + f2[4]*(1-j)**f3[4])/M *(1+dw4); 
    tau1 = 2*(q1[1]+q2[1]*(1-j)**q3[1])/w1 *(1+dtau1); 
    tau2 = 2*(q1[2]+q2[2]*(1-j)**q3[2])/w2 *(1+dtau2); 
    tau3 = 2*(q1[3]+q2[3]*(1-j)**q3[3])/w3 *(1+dtau3); 
    tau4 = 2*(q1[4]+q2[4]*(1-j)**q3[4])/w4 *(1+dtau4); 
...
def gap_sfa_fm(f_,paramater,angles,n_ss,n_ee,random = 0.5,detec = 0,dw=[0,0,0,0],dtau = [0,0,0,0]):
    ...
    origin_freq_sig = sfa(f_,paramater,angles,detec,dw,dtau)
    origin_time_sig = dp.Freq_ifft(origin_freq_sig)
    win_time_sig = dp.win(2.5*pi*dp.qnm_t4(paramater),origin_time_sig,random)
    win_freq_sig = dp.Time_fft(win_time_sig)
