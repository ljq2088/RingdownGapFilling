...
def segment_signal_IMR(transformed_signals, segment_length=Config.segment_length, overlap=Config.overlap):
    segmented_signals = []
...
def segment_signal_IMR(transformed_signals, segment_length=Config.segment_length_IMR, overlap=Config.overlap):
    segmented_signals = []
    step_size = int(segment_length * (1 - overlap))
...
def preprocess_signals(signals, segment_length=Config.segment_length, overlap=Config.overlap):
    ...
    for signal in signals:
        ...
        segmented_signals = segment_signal_IMR(transformed_signals, segment_length=segment_length, overlap=overlap)
