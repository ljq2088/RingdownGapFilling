...
def generate_continuous_mask(batch_size, signal_length,gap_size,start=0):
    ...
    for i in range(batch_size):
        gap_start = torch.randint(start, start+1*gap_size)
        masks[i, gap_start:gap_start + gap_size] = False  # 将连续的部分设为False

    return masks#,gap_start
def generate_continuous_masks_IMR(batch_size, signal_length,gap_size,num_gaps,start=0):
    """
    为每个批次中的信号生成一个在随机位置的连续布尔类型掩码。
    ...
    for i in range(batch_size):
        ...
        for j in range(num_gaps):
            gap_start = np.random.randint(s, s+1*gap_size )
            gap_j=gap_start + np.random.randint(1,gap_size)
            if gap_j<signal_length:

                masks[i, gap_start:gap_j] = False
                s=gap_j+gap_size
