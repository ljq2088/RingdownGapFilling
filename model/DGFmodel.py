...
class Encoder(nn.Module):
    ...
    def compute_token_soft_mask(self, mask_1d):
        """
        mask_1d: [B, 1056] → 返回 token级 soft mask [B, 32]
        每个token长度为64, 按位置取平均
        """
        B = mask_1d.size(0)
        token_soft_mask = torch.zeros(B, self.num_tokens, device=mask_1d.device)
        for i in range(self.num_tokens):
            start = i * (self.token_len // 2)
            end = start + self.token_len
            token_soft_mask[:, i] = mask_1d[:, start:end].float().mean(dim=1)
        return token_soft_mask.clamp(0, 1)  # 防止溢出
    def forward(self, x,qt,mask_1d):
        # 进行 Token Embedding, Convolutional Embedding 和 Position Embedding
        QtTimeMixer=self.QtranTimeMixer(qt)
        B=x.size(0)
        token_soft_mask = self.compute_token_soft_mask(mask_1d)  # [B, 32]
        # if token_soft_mask is not None: 
        #     attn_bias=token_soft_mask.unsqueeze(1).unsqueeze(2) # [B, 1, 1, 32]
        # else:
        #     attn_bias=None
        embedded_tokens = self.totalembedding(x, QtTimeMixer, token_soft_mask, self.mask_token)  # [B, C, 32, D]
        RF = embedded_tokens#.permute(0, 2, 1, 3).reshape(B, self.num_tokens, -1)  # flatten
        # RF=self.totalembedding(x,QtTimeMixer,token_soft_mask,self.mask_token)
       # print(f"RF shape: {RF.shape}")
       # print(f"QtTimeMixer shape: {QtTimeMixer.shape}")
        # Transformer Encoder
        output = self.transformer(RF, key_padding_mask=token_soft_mask)  # 形状为 (batch_size, num_tokens, embedding_dim)
...
class DGF(nn.Module):
    def __init__(self, channels=Config.channels, token_dim=Config.segment_length, embedding_dim=Config.EMBEDDING_dim, num_heads=Config.num_heads, num_layers=Config.num_layers_T, dropout=Config.dropout):
        ...
        self.num_tokens = Config.num_token
        self.token_len = Config.segment_length
        ...
        self.pre_decoder=Decoder(self.inverse_token_embedding)
        self.decoder = Decoder(self.inverse_token_embedding)
        
        
        
        
    def transfer_pretrained_decoder(self):
        """
        将预训练阶段的 pre_decoder 参数复制到 decoder，用于 finetune 初始化。
        """
        self.decoder.load_state_dict(self.pre_decoder.state_dict())

    def forward(self, x,qt,mask_1d):  
        #print(f"input qt shape: {qt.shape}")
        
        encoder_output = self.encoder(x,qt,mask_1d)
        if self.stage == "pretrain":
            output = self.pre_decoder(encoder_output)  # output: [B, 1056]
